...
class DroneEnv(gym.Env):
    ...
    def __init__(self,Encoder, runtime=10., dt=0.001):
        ...
        max_obs = np.ones(21,)   # [Ph_err(3), pos_err(6),rotor_vel_err(4),cntr_cmd(4), Act(4)]
        max_obs = 10*np.ones(self.Encoder.hidden_size,)
    ...
    def reset(self, seed=None):
        ...
        # init_rotors_position = np.random.rand(4)*6.28
        init_rotors_position = np.array([0., 0., 0., 0.])
        ...
        self.Rotors_Vel_err = np.zeros((4,)) 
...
#         only for continuis action
        ...
        self.torchStack = torch.tensor(np.zeros((1, 21, 11)), dtype=torch.float32)
    ...
    def FirstStep(self,ref_cmd = np.array([0., 0., 10., 0., 0., 0.])):
        ...
        for i in range(round(self.damping_time /self.dt)):
            ...
            self.Rotors_Vel_err = rotors_speed_error
            falling = self.Plant.next_timestep(Volt_Input_cmd)
            self.Telem.record(refValuse, rotor_position_err)
            obs = self.get_obs_inner(action=np.zeros(4), )
            self.torchStack = self.StackObs(self.torchStack,obs)
    ...
    def step(self, action , ref_cmd = np.array([0., 0., 10., 0., 0., 0.])):
        ...
        # action = np.concatenate(([0.0],action,[0.0],[0.0]), axis=0).flatten().astype(np.float32)  
        refPhase = 0
        State, rotor_position, time = self.Plant.get_state()
        ref_rotors_speed, refValuse = self.V_cmd.step(ref_cmd, State)
        Volt_Input_cmd, rotor_position_err,rotors_speed_error = self.P_cmd.step(refPhase, ref_rotors_speed)
    ...
    def _get_obs(self, action=np.zeros(4),):
        obs = self.get_obs_inner(action=action,)
        self.torchStack = self.StackObs(self.torchStack, obs)
        new_obs = self.Encoder.featersExtract(self.torchStack).clone().detach().numpy()
        return new_obs.flatten().astype(np.float32)


    def get_obs_inner(self, action=np.zeros(4), ):
        # [Ph_err(3), pos_err(6),rotor_vel_err(4),cntr_cmd(4), Act(4)]
        ...
        curr_state = np.concatenate((ph_err, pos_err, rotor_vel_err, cntr_cmd, act), axis=0).flatten().astype(np.float32)
    ...
    def StackObs(self,ObsTensor, newObs):
        return torch.cat((ObsTensor[:,:,1:], torch.tensor([newObs.reshape(-1,1)])),dim=2)