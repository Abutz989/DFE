...
class PID():
    def __init__(self, Gains=None, dt=0.01, N=100, sat=30):
        ...
        self.Ki = Gains[1]
        self.Kd = Gains[2]
        self.d_err = Dis_Der(Var_len = 1, h=self.dt, sat=30)
        self.I_err = Dis_Intgral(Var_len = 1, h=self.dt, sat=sat)
        self.sat = sat
        self.N = N
    ...
    def culcControl(self, err):
        self.d_err.culcDer(err)
        self.I_err.culcInt(err)
        I = np.copy(self.I_err.Iu)
        u0 = self.Kp*err + self.Ki*I + self.Kd* np.copy(self.d_err.du)
        u = np.clip(u0,-self.sat, self.sat).flatten()[0]
        return u
...
class Dis_Der():
    ...
    def culcDer(self, u):
        ...
        return derivative
    def reset(self):
        self.u = np.zeros((self.Var_len, 4))
        self.du = np.zeros((self.Var_len,))
...
class Dis_Intgral():
    ...
    def reset(self):
        self.u = np.zeros((self.Var_len, 1))
        self.Iu = np.zeros((self.Var_len))
...
class VelCommand(PhysicsSim):
    ...
    def Reset_Controllers(self):
        self.X_cntrl = PID([20, 0.5, 10], self.dt, N=100, sat=30) # 30 = 3g acceleration
        self.Y_cntrl = PID([20, 0.5, 10], self.dt, N=100, sat=30)
        # self.Y_cntrl = PID([20, 4, 10], self.dt, N=100, sat=30)
        self.Z_cntrl = PID([10, 0.0, 5], self.dt, N=100, sat=30)

        # self.Z_cntrl = PID([40, 4, 10], self.dt, N=100, sat=30)
        self.psi_cntrl = PID([20, 0, 15], self.dt, N=100, sat=30)
        self.theta_cntrl = PID([20, 0, 15], self.dt, N=100, sat=30)
        self.phi_cntrl = PID([20, 0, 15], self.dt, N=100, sat=30)
...
#         rotors_speed += rotors_speed_trust
    ...
    def euler_conv(self, x_2dot_d, y_2dot_d, z_2dot_d, psi_d):
        ...
        phi_d = np.arcsin((x_2dot_d * np.sin(psi_d) - y_2dot_d * np.cos(psi_d)) / (d + 1e-7))
